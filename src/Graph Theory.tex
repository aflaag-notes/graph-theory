...
\begin{document}
    ...
    \begin{frameddefn}{Edge maximum graph without $K_r$ subgraphs}
        A graph $G$ is said to be \tbf{edge maximum without $K_r$ subgraph} --- denoted as $\mathrm{ex}(n, K_r)$, for short --- if for any $G'$ such that $\abs{V(G')} = \abs{V(G)}$ and $\abs{E(G')} > \abs{E(G)}$, $G'$ has $K_r$ as subgraph
    \end{frameddefn}
