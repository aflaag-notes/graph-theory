...
\begin{document}
    ...
    We observe that even the outer face is bounded by a triangle.
    ...
    \proofiff{
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}[-,>=stealth,shorten >=1pt,auto,node distance=1.5cm, thick,main node/.style={scale=0.9,circle,draw,font=\sffamily\normalsize}]
                ...
                \node[circle, draw, fill=BlueLagoon] (1) []{};
                \node[circle, draw, fill=BlueLagoon] (2) [below left of = 1]{};
                \node[circle, draw, fill=BlueLagoon] (3) [below right of = 1]{};
                \node[circle, draw, fill=BlueLagoon] (4) [below of = 2]{};
                \node[circle, draw, fill=BlueLagoon] (5) [below of = 3]{};
                \node[circle, draw, fill=BlueLagoon] (6) [below of = 1]{};
                \node[circle, draw, fill=BlueLagoon] (7) [below of = 6]{};

                \draw[-] (1) to (2);
                \draw[-] (1) [BlueLagoon] to (3);
                \draw[-] (1) to (6);
                \draw[-] (2) [BlueLagoon] to (4);
                \draw[-] (2) to (6);
                \draw[-] (3) to (5);
                \draw[-] (3) to (6);
                \draw[-] (4) to (6);
                \draw[-] (4) [BlueLagoon] to (7);
                \draw[-] (5) to (6);
                \draw[-] (5) to (7);

                ;
            \end{tikzpicture}
            \caption{For instance, this graph has $K_4$ as minor, and we are considering as $H'$ the subgraph colored in \tit{blue}.}
        \end{figure}
    ...
    \begin{proof}
        Suppose that $G$ contains $H$ as minor, which implies that there exists a collection $\{X_v \mid v \in V(H)\}$ that satisfied \cref{minor alt def}. Now, for each pair of distinct $u$ and $v$ of $V(H)$ fix an edge $e_{uv} \in E(G)$; since $\Delta(H) = 3$, for any set $X_z$ there can be at most 3 edges of the form $e_{zv_i} \in E(G)$ for $1 \le i \le 3$. TODO \todo{da finire}
    \end{proof}

    Finally, we are ready to discuss Kuratowski's theorem.

    \begin{framedthm}{Kuratowski's theorem}
        A graph $G$ is planar if and only if $G$ does not have $K_5$ nor $K_{3,3}$ as topological minors.
    \end{framedthm}
    
    \begin{proof}
        The observation discussed in \cref{kurat} trivially proves the direct implication of the theorem. Moreover, since TODO \todo{da finire}
    \end{proof}
