...
\begin{document}
    ...
    \begin{proof}
        ...
        We observe that this separation can be extended to be a separation of $G$: if $v_e$ is the vertex constructed after contracting $e$ in $G/e$, then $(A, (B - \{v_e\}) \cup \{x, y\})$ is a separation of $G$. However, we observe that if $v_e \in B - A$, such separation of $G$ has order at most 2, contradicting the 3-connectivity of $G$, meaning that $v_e \in A \cap B$. Moreover, \todo{manca un pezzo} Therefore, for any edge $e := xy \in E(G)$ we can construct a separation $(A_e, B_e)$ of $G$ of order \tit{exactly} 3, such that $x, y \in A_e \cap B_e$. Let $(A_e, B_e)$ be the separation that minimizes $\abs{B_e}$.

        Let $z \in (A_e \cap B_e) - \{x, y\}$ be the third vertex of $A_e \cap B_e$; if $z$ has no neighbors in $B_e - A_e$, then by removing $e$ from $G$ we would disconnect the graph, contradicting the assumption of 3-connectivity. This means that there is a vertex $u \in B_e - A_e$ such that $z \sim u$. Let $e' := zu$, and consider a non-trivial separation $(A_{e'}, B_{e'})$ of $G$ of order \tit{exactly} 3 such that $z, u \in A_{e'} \cap B_{e'}$.

        TODO \todo{drawing of separations}
        ...
        \claim{
            ...
            By way of contradiction, suppose that $(A_e - B_e) - A_{e'} \neq \varnothing$, and fix a vertex $w \in (A_e - B_e) - A_{e'}$. Since $(A_e, B_e)$ is a separation, we know that $w \nsim u$. Moreover, since $(A_{e'}, B_{e'})$ is a separation, $w$ can only be adjacent to vertices in $(A_{e'} \cap B_{e'}) - B_e$. Therefore, by removing $(A_{e'} \cap B_{e'}) - B_e$ from $G$ we disconnect $w$ from $u$ necessarily --- since $(A_e \cap B_e) - A_{e'} = \varnothing$ by the previous claim. \todo{what if w sim z?} Moreover, since $(A_{e'} \cap B_{e'}) - B_e$ contains one vertex by choice on the order of $(A_{e'}, B_{e'})$, we get a contradiction on 3-connectivity of $G$ $\lightning$.
        }

        \claim{
            $(A_{e'} \cap B_{e'}) - B_e = \varnothing$.
        }{
            Consider the set $B_e - (A_e \cup A_{e'})$; since we picked $(A_{e'}, B_{e'})$ to be a non-trivial separation, we know that $B_{e'} - A_{e'} \neq \empty$. Therefore, the previous two claims imply that $B_e - (A_e \cup A_{e'}) \neq \varnothing$.

            Now, by way of contradiction suppose that $(A_{e'} \cap B_{e'}) - B_e \neq \varnothing$; by choice of $(A_{e'}, B_{e'})$, we know that $A_{e'} \cap B_{e'}$ only contains 3 vertices. TODO \todo{il finale non ha senso}
        }

        Therefore, by these three claims we get that $B_{e'} \subseteq B_e$, but because $x \in (A_{e'} - B_{e'}) \cap (A_e \cap B_e)$ then $B_{e'} \subsetneq B_e$, which contradicts the choice of the separation $(A_e, B_e)$ that minimized $\abs{B_e}$ $\lightning$.
    \end{proof}

    From this theorem, we can derive the following corollary, which gives the characterization of 3-connected graphs that we previously anticipated.

    \begin{framedcor}{}
        If a graph $G$ is 3-connected, and $n \ge 5$, there exists a sequence $G_1, \ldots, G_k$ such that
    ...
    Unformally, we could say that every 3-connected graph can be constructed by repeatedly \curlyquotes{unconstructing} edges.
