...
\begin{document}
    ...
    \begin{frameddefn}{Induced subgraph}
        Given a graph $G = (V, E)$, and a set of vertices $S \subseteq V$, then $G[S]$ represents the \tbf{subgraph induced by $S$ on $G$}, and it is obtained by removing from $G$ all the nodes of $V - S$ --- and their corresponding edges.
    \end{frameddefn}

    In other words, a subgraph $G' = (V', E')$ of $G$ is \tbf{induced} if every edge of $G$ with both ends in $V$ is an edge of $V'$. We observe that this definition is \tit{stricter} than the previous one: in fact, the last graph is \tit{not} an example of an induced subgraph, but the following is:
    ...
    \begin{proof}
        ...
        Note that, if by removing $v$ from $T$ we disconnect $x$ and $y$, it must be that $v$ lies in $P$. Moreover, since $v$ is in $T$ but not in $T[V(T) - \{v\}]$, while both $x$ and $y$ are also in $T[V(T) - \{v\}]$, it must be that $v$ is an \tit{internal} node of $P$, i.e. $v \neq x, y$, which implies that $\deg(v) \ge 2$ by definition of path, contradicting the hypothesis for which $v$ was a leaf $\lightning$.
    \end{proof}
    ...
    \proofind{
        ...
        We will prove the statement for a tree $T$ that has $n = k$ nodes. Note that, since $n = 1$ is the base case, we can assume that $n = k \ge 2$, hence by \cref{leaf existence} $T$ contains at least one leaf. Let this leaf be $v$; then, by \cref{tree without leaf} it holds that $T[V(T) - \{v\}]$ is still a tree, and clearly $T[V(T) - \{v\}]$ has $k - 1$ nodes, which implies that we can apply the inductive hypothesis on $T[V(T) - \{v\}]$, i.e. $$\abs{E(T[V(T) - \{v\}])} = \abs{V(T[V(T) - \{v\}])} - 1 = k - 1 - 1 = k - 2$$ However, note that $v$ is a leaf, concluding that $$m = \abs{E(T)} = \abs{E(T[V(T) - \{v\}])} + 1 = k - 2 + 1 = k - 1 = n - 1$$
    }
    ...
    \proofind{
        ...
        \claim{
            ...
            Starting with $V := V(C)$, as long as $H[V(H) - V]$ contains vertices of degree at most 1, move them into $V$; let $M$ be the number of moved vertices. Then, since we are adding vertices to $V$, we have that $\abs{\mathrm{cut}(V)} \le \abs{\mathrm{cut}(C)} - M$. We observe that, by construction $\delta(H[V(H)- V) \ge 2$, and the number of vertices that have degree 2 in $H[V(H) - V]$ is at most $\abs{\mathrm{cut}(V)} \le \abs{\mathrm{cut}(C)} - M$.
    ...
    \proofstrind{
        ...
        Suppose that $\delta < 2k$, i.e. $\delta \le 2k - 1$, implying that there exists a vertex $v$ such that $\deg(v) \le 2k - 1$, and consider $G[V(G) - \{v\}]$. Since this subgraph has $n - 1$ vertices, and \todo{WHAT last ineq} $$\abs{E(G[V(G) - \{v\}])} \ge \abs{E(G)} - (2k - 1) \ge (2k - 1) ((n - 1) - k)$$ Moreover, since $n \ge 2k + 1 \iff n - 1 \ge 2k$ and $G[V(G) - \{v\}]$ has $n - 1$ vertices, the latter has at least $2k$ vertices, hence we can apply the inductive hypothesis on it and obtaining a $k$-connected subgraph to prove the statement trivially. Therefore, we may assume that $\delta \ge 2k$.
        ...
        and in particular, the last assumption implies that we may assume there is an $A$-$B$ separation $(X, Y)$ of $G$ having order $\abs{X \cap Y} \le k - 1$, where $X - Y , Y - X \neq \varnothing$ --- for some subsets $A, B \subseteq V(G)$.
        ...
        Finally, by the claim we can apply the inductive hypothesis on at least one between $X$ or $Y$, concluding the proof.
    }
    ...
    \begin{framedthm}{Bollobás-Thomason theorem}
        There exists a constant $c$ such that if $G$ has at least $ck^2n$ edges, then $G$ has $K_k$ as topological minor.
    \end{framedthm}

    TODO $2 \le c \le 10$ \todo{interlude}
    ...
    \begin{proof}
        ...
        \begin{itemize}
            ...
            \item For $p = 4$, we have that $\abs{E(G)} \ge (4 - 2)n - \binom{4 - 1}{2} + 1 = 2n - 3 + 1 = 2n - 2$. We proceed by induction on $n$.

                We observe that if $n = 2$ then $\abs{E(G)} \ge 2 \cdot 2 - 2 = 2$ which is impossible with only 2 vertices, and similarly if $n = 3$ then $\abs{E(G)} \ge 2 \cdot 3 - 2 = 4$ which is impossible with only 3 vertices. Hence, to satisfy the edge bound $n$ must be at least 4. Therefore, for the base case assume that $n = 4$, which means that $\abs{E(G)} \ge 2 \cdot 4 - 2 = 6$, which implies that $G = K_4$.

                Now, assume that the property is true for a graph of $n - 1$ vertices and consider a graph $G$ of $n$ vertices such that $n \ge 5$. Suppose that $\abs{E(G)} > 2n - 2$, and fix an edge $e \in E(G)$. Now, consider the graph $G - \{e\}$: clearly, this graph has still $n$ vertices but since we removed an edge it holds that $\abs{E(G - \{e\})} > 2n - 2 - 1 > 2n - 3 \iff \abs{E(G - \{e\})} \ge 2n - 2$, therefore we can apply the \todo{THIS IS FALSE}

                Therefore, we may assume that $\abs{E(G)} = 2n - 2$. Fix an edge $e \in E(G)$, and consider the contracted graph $G/e$: this graph has $n - 1$ vertices, hence if $\abs{E(G/e)} \ge 2(n - 1) - 2$ by the inductive hypothesis the theorem holds trivially. Thus, we may assume that $\abs{E(G/e)} < 2(n - 1) - 2 = 2n - 4 \iff \abs{E(G/e)} \le 2n - 5$, and since we are assuming that $\abs{E(G)} = 2n - 2$, this means that by contracting $e$ we removed at least 3 edges from $G$ --- one of which is $e$ itself. Therefore, we know that in $G$ we have the following structure
