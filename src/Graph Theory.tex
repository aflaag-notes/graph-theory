...
\begin{document}
    ...
    \begin{proof}
        ...
        \textbf{Claim}: There are a sequence of sets $X_1, \ldots, X_{2t -3}$ and a sequence of vertices $x_1, \ldots , x_{2t - 3}$ such that for each $i \in [2t - 2]$ it holds that
    ...
    \begin{framedprob}{}
        Given a set $\mathcal I$ of graphs, we say that $H_1, \ldots, H_k$ are \tit{forbidden minors} for $\mathcal I$ when $G \in \mathcal I$ if and only if $G$ does not have any $H_i$ as minor. Find the forbidden minors for the set $\mathcal F$ f forests.
    \end{framedprob}
    ...
    \begin{framedprob}{}
        A graph is said to be \tit{complete multipartite} if there exist a partition $X_1, \ldots, X_k$ of $V(G)$ such that
    ...
    Consider a graph $G$, and a vertex $v \in V(G)$; to \tbf{expand} $v$ means to add another vertex $v'$ to $V(G)$ and
    
    \begin{itemize}
        \item add an edge $\{v, v'\}$ to $E(G)$
        \item add edges $\{v', x\}$ to $E(G)$ for each $x \in \mathcal N (v)$
    \end{itemize}

    TODO \todo{example}

    \begin{framedlem}{}
        % Given a perfect graph $S$, if a graph $G'$ is obtained by repeatedly expanding vertices starting from $S$, then $G'$ is perfect.
        TODO
    \end{framedlem}
    ...
    \proofind{
        ...
        % For the base case, we have that $\abs{V(S)} = \abs{V(G')}$, therefore we applied 0 vertex expansions to obtain $G'$, thus the latter is trivially perfect by perfection of $G$
    }{
        % Assume that the statement holds for a graph $G'$ on $n - 1$ vertices.
    }{
        % Consider a graph $G$ on $n - 1$ vertices obtained by repeatedly expanding vertices starting from $S$, inductively assume that $G$ is perfect, and expand one of its vertices to obtain $G'$ on $n$ vertices. Now, fix an induced subgraph $H$ of $G$; then
