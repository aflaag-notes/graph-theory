...
\begin{document}
    ...
    In the first section of this chapter, we will explore the bounds on the number of \tbf{edges} that force any graph to contain interesting substructures. In particular, we define $\mathrm{ex}(n, H)$ to be the maximum number of edges in an $n$-vertex graph that does not contain a given graph $H$ as subgraph.
    ...
    \begin{framedcor}{}
        $\mathrm{ex}(n, K_r) = t(n, r - 1)$
    \end{framedcor}

    \begin{proof}
        By the previous theorem, if a graph $G$ has $n$ vertices and more than $t(n, r - 1)$ edges, it clearly cannot be $T(n, r - 1)$, therefore by the theorem it must contain $K_r$ as subgraph.
    \end{proof}
    ...
    \begin{frameddefn}{Split graph}
        A \tbf{split graph} is a graph whose vertices can be partitiones into a \tit{clique} and an independent set. A \tbf{complete split graph} is a split graph where the independent set is fully connected to the clique.
    \end{frameddefn}
    ...
    What about other values of $t$? Using a similar idea of this result, it can be proven that $R(5, 4) \le 25$, which immediatly provides an upper bound for $R(5) \le 50$. However, we still don't know the exact value of $R(5)$: in 1989 \textcite{exoo} proved that $R(5) \ge 43$, and in 2024 \textcite{angeltveit} proved that $R(5) \le 46$.
    ...
    \begin{framedprob}{}
        Prove that $\mathrm{ex}(n, P_4) \le n$.
    \end{framedprob}
    ...
    \begin{framedprob}{}
        Show that $\mathrm{ex}(n, K_{1,3}) = n$.
    \end{framedprob}

    \solution{
        First, we observe that any $C_n$ cycle graph on $n$ vertices has $n$ edges but does not contain $K_{1,3}$ as subgraph, therefore $\mathrm{ex}(n, K_{1,3}) \ge n$.
        ...
        In particular, if $\Delta(G) \ge 3$, it implies that $K_{1,3}$ must be a subgraph of $G$, concluding that $\mathrm{ex}(n, K_{1,3}) \le n$.
    }
    ...
    \solution{
        We observe that if $n < t + 1 \iff n \le t$ it is impossible to have $K_{1, t}$ as subgraph since it requires at least $t + 1$ vertices in the graph. This means that $$n \le t \implies \mathrm{ex}(n, K_{1,t}) = \binom{n}{2} \le \binom{t}{2}$$ Therefore, we may assume that $n \ge t + 1$.

        Note that to guarantee that $K_{1,t}$ is a subgraph of $G$, it suffices to guarantee that $G$ contains a vertex of degree at least $t$. Hence, the largest number of edges $G$ can have must be such that every vertex has degree $t - 1$, i.e. the sum of the degrees of $G$ is $n (t - 1)$. Hence, by the Handshaking lemma we get that $$2 \abs{E(G)} = \sum_{v \in V(G)}{\deg(v)} = n (t - 1) \iff \abs{E(G)} = \dfrac{n}{2}(t - 1)$$ We are going to prove that this is in fact the exact value of $\mathrm{ex}(n, K_{1,t})$.
        ...
        \claim{
            For $t \ge 4$ it holds that $\mathrm{ex}(n, K_{1,t}) \le \tfrac{n}{2}(t  - 1)$.
        }{
        ...
        Together with the previous counterexample, we get that $$n \ge t + 1 \implies \mathrm{ex}(n, K_{1, t}) = \dfrac{n}{2}(t - 1)$$
    }
