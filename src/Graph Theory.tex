...
\begin{document}
    ...
    \proofstrind{
        ...
        \claim{
            ...
            By way of contradiction, suppose that $\abs{E(G[X])} \le (2k - 1)(\abs - k) - 1$ and $\abs{E(G[Y])} \le (2k - 1)(\abs Y - k) - 1$. We observe that $$$$ Thus, we have that
            \begin{equation*}
                \begin{split}
                    \abs{E(G)} &\le \abs{E(G[X])} + \abs{E(G[Y])} \\
                               &\le \abs{E(G[X])} \le (2k - 1)(\abs - k) - 1 + \abs{E(G[Y])} \le (2k - 1)(\abs Y - k) - 1 \\
                               &= (2k - 1) (\abs X + \abs Y - 2k) - 2 \\
                               &TODO
                \end{split}
            \end{equation*}

            TODO \todo{da finire (anche lo split)}
        }
    ...
    \begin{proof}
        ...
        \begin{itemize}
            ...
            \item For $p = 3$, we have that $\abs{E(G)} \ge (3 - 2) n - \binom{3 - 1}{2} + 1 = n - 1 + 1 = n$. Thus, by \cref{m n - 1} this implies that $G$ is not a tree (nor a forest), and in particular it is not acyclic, meaning that it contains at least one cycle $C$. Lastly, if we contract all but 3 edges on the cycle we obtain $K_3$.
