...
\begin{document}
   ...
   % }
    ...
    Is this the best result we can achieve through planarity? This upper bound on planar graph can be actually improved thorugh the following proposition.
    ...
    What are example of perfect graphs? A trivial class of perfect graphs are \tit{cliques}, since each induced subgraph of a clique is still a clique.
    ...
    \solution{
        ...
        \claim[Claim 3]{
            ...
            Now, consider $G[X]$, and by way of contradiction suppose that there is no $x \to y$ path inside $G[X]$; then, $x$ and $y$ must live in different components of $G[X]$, which means that it is always possible to permute the colors in the component containing say $y$ such that $c_X(x) \neq c_X(y)$ for any proper 3-coloring $c_X$ of $G[X]$, contradicting the fact that every coloring of $G[X]$ must force $x$ and $y$ to have the same color. This implies that $G[X]$ has an $x \to y$ path.

            By repeating the same argument, we get that $G[Y]$ also contains an $x \to y$ path, therefore by contracting this path to a single edge say inside $G[Y]$ we obtain a new graph $H$; thus \todo{da finire}

            % \begin{itemize}
            %     \item if $H$ has no $K_4$ as minor, then since $\abs{V(H)} < \abs{V(G)}$ then $H$ is 3-colorable
            % \end{itemize}
        }
