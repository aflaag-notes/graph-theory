...
\begin{document}
    ...
    \proofind{
        ...
        Consider an $A$-$B$ separation $(X, Y)$ of $G - \{xy\}$, and assume that the two vertices $x \in X - Y$ and $y \in Y - X$. \todo{stanno a cavallo perché altrimenti non danno problemi}
    ...
    \begin{proof}
        By way of contradiction, let $C$ be the cycle that contains as many vertices $x_i$ as possible; if $x_1, \ldots, x_k \in V(C)$, the theorem is trivially verified, hence we may assume that at least one vertex $x_i$ is not in $V(C)$, and w.l.o.g. assume that $x_k \notin V(C)$. By \cref{kconn} we know that $\delta \ge k$, and by \cref{min deg 2} we know that $\abs{V(C)} \ge k + 1$. Hence, by the previous corollary we know that exist $k$ $\{x_k\}$-$V(C)$ paths that only intersect in $x_k$.
    ...
    \section{Subdivision and suppression}
    ...
    \begin{proof}
        If $G$ is 3-connected, then by \cref{kconn} we know that $\delta_G \ge 3$, and by \cref{min deg 2} this means that in $G$ there is a cycle $C$. Hence, consider a vertex $v \notin V(C)$ --- note that this vertex must exist by 3-connectivity of $G$. Note that $\delta_G \ge 3 \implies \delta_{G[V(G) - \{v\}]} \ge 2$, and by applying \cref{min deg 2} on $G[V(G) - \{v\}]$ we have that the latter contains a cycle as well. \todo{da finire}
    \end{proof}
    ...
    \begin{proof}
        ...
        and lastly, since $V(T) = V(G) = n$, the height of $T$ is $\ceil{\log n} - 1$ \todo{questa non è l'altezza e lo statement è sbagliato}, which means that such a cycle must have length $$\ceil{\log n} - 1 + \ceil{\log n} - 1 + 1 = 2 \ceil{\log n} - 1$$
    \end{proof}

    \begin{framedlem}{}
        Given a graph $G$, if $G$ contains a topological minor of a 3-regular graph $H$, where $V(H) \ge c' k \log(k + 1)$ for some constant $c'$, then $G$ contains $k$ disjoint cycles.
    \end{framedlem}

    \proofind{
        We proceed by induction on $k$.
    }{
        For $k = 1$, if $G$ contains a topological minor of a 3-regular graph $H$, then $\delta_H \ge 3$ meaning that $H$ contains a cycle for \cref{min deg 2}, and its subdivision contained in $G$ will also contain a cycle --- since the subdivision operation only adds edges.
    }{
        Assume that the statement holds for $k - 1$.
    }{
        Let $C$ be the shortest cycle of $H$; by the previous proposition, we know that $\abs{V(C)} \le 2 \log(c'k\log(k +1)) + 2$ --- adding 2 takes care of the rounding error introduced by the \tit{ceiling} operation --- and note that $$\abs{V(C)} \le 2 \log(c'k \log(k + 1)) + 2 = 2 \log c' + 2 \log k + 2 \log \log(k + 1) + 2$$ Now, consider the induced subgraph $H - C$: by 3-regularity \todo{sta roba non funziona}
    }

    Thanks to this lemma, it is possible to prove the following result, first presented by \textcite{erdos} in 1965.
    ...
    \begin{framedthm}{Erdős-Pósa theorem}
        ...
        \begin{itemize}
            ...
            \item there is a subset $X \subseteq V(G)$ that intersects all cycles such that $\abs X \le c k\log k$
        \end{itemize}
    \end{framedthm}

    \begin{proof}
        TODO \todo{l'ha fatta incompleta, forsa la prossima volta la rifà}
    \end{proof}
