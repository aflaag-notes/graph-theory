...
\begin{document}
    ...
    We say that a graph is $k$-colorable if $k$ colors suffices to provide a proper coloring of such graph. We observe that such a function always exists, i.e. the \tbf{trivial coloring}: it suffices to color each vertex with a different color. Therefore, we are interested in finding a proper coloring of a graph that \tit{minimizes} the number of colors used.
