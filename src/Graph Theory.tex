...
\begin{document}
    ...
    \subsection{Eulerian tours and Hamiltonian cycles}
    ...
    This suggests that the presence of augmenting paths in a graph is a \tit{sufficient} condition for a matching \tit{not} to be maximum, but we can actually prove that it is also \tit{necessary}, as stated in the following theorem.
    ...
    \begin{proof}
        ...
        \claim{
            ...
            Consider an edge $ab \in E(G)$ such that $a \in A$ and $b \in B$; we have three cases.

            \begin{itemize}
                \item By way of contradiction, assume that $ab$ is disjoint from $M^*$; then, either $a$ or $b$ are endpoints of an edge $e$ that is disjoint from $M$ by definition, therefore $M^* \cup \{e\}$ is still a matching, but of larger cardinality than $\abs{M^*}$ $\lightning$. Hence, $ab$ is not disjoint from $M^*$, i.e. there exists say $a' \in A$ such that $a'b \in M$, meaning that $$a \ \{a, b\} \ b \ \{b, a'\} \ a'$$ is an $M^*$-alternating path that starts in TODO \todo{there is a hole here}
                \item TODO
            \end{itemize}
        }
    \end{proof}
    ...
    \begin{proof}
        ...
        Note that, in the general case we would consider the path $P' := aTy \cup yP$, however this is not guaranteed to be a path. The complete proof leverages the fact that $G$ is bipartite in order to prove that $P'$ is indeed a path, but it is very technical and outside the scope of these notes.
    \end{proof}
    ...
    \subsection{Perfect and stable matchings}
    ...
    In fact, \href{https://en.wikipedia.org/wiki/Hall%27s_marriage_theorem}{Hall's marriage theorem} --- proved by \textcite{hall} in 1935 --- provides a characterization of bipartite graphs which have a perfect matching.

    \begin{framedthm}{Hall's marriage theorem}
        Given a bipartite graph $G$, bipartitioned into $(A, B)$ such that $\abs A = \abs B$, then $G$ admits a perfect matching if and only if $$\forall S \subseteq A \quad \abs S \le \abs{\mathcal N (S)}$$
    \end{framedthm}

    \begin{proof}
        The direct implication is trivially true by definition of matching. We will prove the converse implication by contrapositive. Suppose that the bipartite graph $G$ does not admit any perfect matching. TODO \todo{da finire}
    \end{proof}
    ...
    If the family of preference functions $\{w_v\}_{v \in V}$ is such that $$w_a(b) > w_b(b') \land w_b(a) > w_b(a')$$ which implies that \underline{both} $a$ and $b$ would prefer to \tit{switch} their current matched vertex --- then $M$ is \tit{not} stable.
    ...
    \begin{framedthm}{Gale's theorem}
    ...
    \begin{framedprob}{}
        Let $G = (V, E)$ be a graph of $n$ vertices, where $n \ge 2$. Show that there must exist two vertices $x, y \in V$ such that $\deg(x) = \deg(y)$.
    \end{framedprob}
