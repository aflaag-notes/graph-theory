...
\begin{document}
    ...
    \begin{proof}
        By way of contradiction, if $G$ does not admit $k$ disjoint $A$-$B$ paths, then by \cref{menger} there can be $A$-$B$ separations of order at most $k - 1$. \todo{non mi torna} TODO \todo{CHE SENSO HA DIRE MAX NUM OF AB PATHS}
    \end{proof}
    ...
    \begin{proof}
        ...
        \begin{itemize}
            \item The two vertices are not adjacent, i.e. $x \nsim y$. By \cref{kconn}, we know that $\abs{\mathcal N(x)} , \abs{\mathcal N(y)} \ge k$, hence by the previous proposition we there exist $k$ disjoint $\mathcal N(x)$-$\mathcal N(y)$ paths, which form $k$ internally disjoint $\{x\}$-$\{y\}$ paths by $k$-connectivity of $G$. \todo{impreciso, che significa?}
            \item The two vertices are not adjacent, i.e. $x \nsim y$. By $k$-connectivity of $G$, we know that $G - \{xy\}$ is $(k-1)$-connected \todo{this is false by definition} and TODO
        \end{itemize}
    \end{proof}
    ...
    \begin{framedlem}{}
        Given a graph $G$, if there is a subgraph $H$ of $G$ such that $H$ is 3-regular on at least $c' k \log(k + 1)$ vertices for some constant $c'$, then $G$ contains $k$ disjoint cycles.
    \end{framedlem}

    \proofind{
        We proceed by induction on $k$.
    }{
        If $k = 1$, and $G$ contains a subgraph $H$ 3-regular on $$c' k \log(k + 1) = c' \cdot 1 \cdot \log 2 = c'$$ vertices, then $\delta_H \ge 3$, meaning that $H$ contains a cycle for \cref{min deg 2}.
    }{
        TODO \todo{da finire}
    }{
        % Let $C$ be the shortest cycle of $G$; by the previous proposition, we know that 
        TODO \todo{ha detto che forse la rifà perché è imprecisa}
    }
