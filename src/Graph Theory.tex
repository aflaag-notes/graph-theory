...
\begin{document}
    ...
    \begin{proof}
        ...
        Lastly, since $G^M$ has $\abs M$ edges by definition, any vertex cover defined on $G^M$ has to contain at least $\abs M$. This implies that no vertex cover $S$ of $G$ smaller than $\abs M$ can exist, because $S$ will have to cover at least the edges in $M$.
    \end{proof}
    ...
    Moreover, if the graph is bipartite the theorem is actually stronger, as proved by \textcite{konig} in 1931.
    ...
    \begin{proof}
        ...
        \claim{
            ...
            \begin{itemize}
                \item By way of contradiction, assume that $ab$ is disjoint from $M^*$; then, either $a$ or $b$ are endpoints of an edge $e$ that is disjoint from $M$ by definition, therefore, $M^* \cup \{e\}$ is still a matching of larger cardinality than $\abs{M^*}$ $\lightning$.
                \item TODO
            \end{itemize}
        }
    \end{proof}
