...
\begin{document}
    ...
    \begin{proof}
        Let $P$ be an $M$-augmenting path that starts in $a$ and minimizes the edges in $E(P) - E(T)$. By way of contradiction, assume that $E(P) - E(T) = \varnothing$, i.e. $P$ is completely contained in $T$. \todo{non ha senso??}
    \end{proof}
    ...
    If the family of weight functions $\{w_v\}_{v \in V}$ is such that $$w_a(b) > w_b(b') \land w_b(a) > w_b(a')$$ which implies that \underline{both} $a$ and $b$ would prefer to \tit{switch} their current matched vertex --- then $M$ is \tit{not} stable.
