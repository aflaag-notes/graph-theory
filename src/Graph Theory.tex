...
\begin{document}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[-,>=stealth,shorten >=1pt,auto,node distance=1.75cm, thick,main node/.style={scale=0.9,circle,draw,font=\sffamily\normalsize}]
            ...
            ;
        \end{tikzpicture}
        \caption{For instance, in this graph $G$ have that $G[\{3, 4, 5\}]$ is an induced cycle but $G[\{1, 2, 3, 4, 5, 6\}]$ is not, because $1 \sim 6$.}
    \end{figure}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[-,>=stealth,shorten >=1pt,auto,node distance=1.75cm, thick,main node/.style={scale=0.9,circle,draw,font=\sffamily\normalsize}]
            ...
            ;
        \end{tikzpicture}
        \caption{For instance, the edge $\{2,3\}$ is a \tit{chord} of this cycle graph.}
    \end{figure}
    ...
    \solution{
        ...
        Therefore, we are assuming that there is at least a tree of the collection containing $\ell$, and each such tree must contain at least another vertex. Therefore, by connectivity of each subtree we know that there is one vertex --- say $u$ --- such that $\ell \sim u$ which in turn implies that $\forall i \in [k] \quad \ell \in V(T_i) \implies u \in V(T_i)$. Now, for each $i \in [k]$ let $T_i'$ indicate the subtree $T_i - \{\ell\}$; the previous observation implies that $\forall i, j \in [k] \quad i \neq j, \ell V(T_i) \cap V(T_j) \implies u \in V(T_i) \cap V(T_j) \implies u \in V(T_i') \cap V(T_j')$. Now, consider the collection $T_1', \ldots, T_k'$; this observation implies that for each $i, j \in [k]$, if $i \neq j$ then
    ...
    Note that if $X$ is an $A$-$B$ hitting set, then $A \cap B \subseteq X$, otherwise any single vertex in $A \cap B$ would represent an $A$-$B$ trivial path that does not pass through the hitting set $X$. Moreover, consider an $A$-$B$ hitting set $X$ of a graph $G$; 
    ...
    This proposition clearly implies that the maximum number of disjoint $A$-$B$ paths is upper bounded by the cardinality of the minimum $A$-$B$ hitting set. However, hitting sets are not easy to work with, and more convenient way to reason about $A$-$B$ paths are \tbf{graph separations}.
    ...
    From the very definition, we see that any $A$-$B$ path must pass through $X \cap Y$, since there are no edges \curlyquotes{across} $X$ and $Y$. Moreover, given a graph $G$, two sets $A, B \subseteq V(G)$, and an $A$-$B$ separation $(X, Y)$ of $G$, we observe that $G - (X \cap Y)$ is disconnected. Furthermore, consider the following property.
    ...
    This property directly implies that hitting sets and graphs separations can be treated equivalently. Hence, because of the previous proposition we derive the following observation.

    \begin{frameddefn}[label={menger prop 3}]{}
    ...
    In 1927 \textcite{menger} proved that the lower bound also holds. To prove his result, we are going to prove a stronger version of his theorem
    ...
    We will use \tit{extensively} Menger's theorem and its implications in the following chapters.
    ...
    Nonetheless, the Gallai-Milgram theorem still has applications in various branched of mathematics. For instance, it can be used to prove a result first derived by \textcite{dilworth} in 1950 involving \tbf{posets}.
    ...
    \begin{frameddefn}{Chain}
        Given a poset $(S, \prec)$, a \tbf{chain} is a sequence of pairwise comparatble elements of $S$ w.r.t. $\prec$. In other words, $C \in S$ is an anti-chain if it holds that $$\forall s, s' \in C \quad s \prec s' \land s' \prec s$$
    \end{frameddefn}

    \begin{frameddefn}{Anti-chain}
        Given a poset $(S, \prec)$, an \tbf{anti-chain} is a set of pairwise uncomparable elements of $S$ w.r.t. $\prec$. In other words, $A \in S$ is an anti-chain if it holds that $$\forall s, s' \in A \quad s \nprec s' \land s' \nprec s$$
    \end{frameddefn}
   ...
   % }
    ...
    \begin{proof}
        ...
        \textbf{Claim}: There are a sequence of sets $X_1, \ldots, X_{2t -3}$ and a of sequence vertices $x_1, \ldots , x_{2t - 3}$ such that for each $i \in [2t - 2]$ it holds that
    ...
    \begin{framedprob}{}
        Show that for any choice of the coloring function $\func{c}{E(K_{17})}{\{\mbox{R, G, B}\}}$ there are three vertices
    \end{framedprob}
    ...
    Is this the best result we can achieve through planarity? TODO \todo{da finire}
    ...
    So far, we only discussed \tit{upper bounds} on the chromatic number, but what about \tit{lower bounds}? Consider the following definition.
    ...
    However, $\omega(G)$ can be significantly lower than $\chi(G)$, in fact TODO \todo{find paper and add name in next thm} First, consider the following proposition.
    ...
    Let's look deeper into the relationship between $\chi(G)$ and $\omega(G)$: for which graphs it holds that $\chi(G) = \omega(G)$? A rather uninteresting answer to this question would be any graph $H$ on $n$ vertices, together with a $K_n$ disconnected from $H$ itself, in fact we would have that $$n = \omega(G) \le \chi(G) \le n \implies \omega(G) = \chi(G)$$ This means that asking for a graph $G$ such that its chromatic and clique number coincide does not provide interesting answers, hence we need a strictier definition.
    ...
    What are example of perfect graphs? A trivial class of perfect graphs are \tbf{cliques}, since each induced subgraph of a clique is still a clique.

    \begin{framedprop}{}
        Cliques are perfect.
    \end{framedprop}

    Another example of perfect graphs is given by \tbf{bipartite graphs}.
    ...
    A trivial example of chordal graphs are \tbf{cliques}.
    ...
    We are going to prove that interval graphs by proving the following proposition.
    ...
    %
    %     \begin{itemize}
    %         \item Suppose that $\abs{X \cap Y} = 0$; if $G$ has no $K_4$, then nor $G[X]$ neither $G[Y]$ have $K_4$ as minor, however $\abs{V(G[X])}, \abs{V(G[Y])} < \abs{V(G)}$ which implies that $G[X]$ and $G[Y]$ are 3-colorable by minimality of $G$, thus $G$ is 3-colorable by the following proper coloring $$\forall z \in V(G) \quad c(z) = \soe{ll}{c_X(z) \quad z \in X \\ c_Y(z) \quad z \in Y}$$ contradicting the fact that it required at least 4 colors to be proper colored by definition $\lightning$.
    %         \item Suppose that $\abs{X \cap Y} = \{v\}$; by the same argument $G[X]$ and $G[Y]$ are 3-colorable by two proper coloring $c_X$ and $c_Y$. If $c_X(v) = c_Y(v)$ then $G$ is trivially 3-colorable, so we may assume that $c_X(v) \neq c_Y(v)$; then, it suffices to permute the colors of $c_Y$ such that $c_X(v) = c_Y(c)$ and consider the same coloring function described in the previous case, to construct a 3-coloring for $G$ $\lightning$.
    %         \item TODO \todo{da finire}
    %     \end{itemize}
    % }
