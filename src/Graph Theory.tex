...
\begin{document}
    ...
    Up until now, we focused on finding conditions on the number of \tbf{edges} which guaranteed the existence of particular structures inside graphs. A natural extension of this idea is to finding conditions on the number of \tbf{vertices}. In 1917, \textcite{ramsey} published a groundbreaking paper in which he proved the existence of the so-called \tbf{Ramsey number}.

    \begin{frameddefn}{Ramsey number (first version)}
        Given a value $t \in \N$, the \tbf{Ramsey number} $R(t)$ is the number of vertices such that every graph with at least $R(t)$ vertices has $K_t$ as subgraph or contains an independent set of size $t$.
    \end{frameddefn}
    ...
    \begin{proof}
        We are going to construct a sequence of sets of vertices.

        \begin{itemize}
            \item Let $X_1 := V(G)$, and fix $x_1 \in X_1$ arbitrarily
            \item Let $X_2$ to be the larger between $\mathcal N(x_1)$ and $V(G) - (\mathcal N(x_1) \cup \{x_1\})$; fix $x_2 \in X_2$ arbitrarily.
            \item In general, suppose that we have defined $X_i$ and $x_i$ for $i \in [1, k]$. Fix $X_{k + 1}$ to be the larger between $\mathcal N(x_k) \cap X_k$ and $X_k - (\mathcal N(x_k) \cup \{x_k\})$
        \end{itemize}

        TODO \todo{da finire}
        % We are going to define a sequence of sets $X_i \subseteq V(G)$ for vertices $x_i \in V(G)$ where $1 \le i \le 2t - 2$, such that

    \end{proof}

    Trivially, $R(1) = 1$ since a single vertex is an independent set of size 1, and $R(2) = 2$ since a graph with two vertices can only be either $K_2$ or an independent set of size 2. What about other values?
