...
\begin{document}
    ...
    % Menger’s theorem, a fundamental result in graph theory, was proved by the Austrian mathematician Karl Menger in 1927. It establishes a deep connection between connectivity and separation in graphs, stating that the minimum number of vertices (or edges) needed to separate two non-adjacent vertices equals the maximum number of pairwise internally disjoint paths between them. Menger’s work laid the foundation for modern connectivity theory and influenced later results, such as the max-flow min-cut theorem. While the precise historical development of the theorem is not widely documented, it emerged from Menger’s broader research in combinatorial mathematics and influenced key figures like Paul Erdős in advancing graph theory. \todo{modifica}
