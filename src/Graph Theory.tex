...
\begin{document}
    ...
    \begin{figure}[H]
        ...
        \begin{tikzpicture}[-,>=stealth,shorten >=1pt,auto,node distance=1.5cm, thick,main node/.style={scale=0.9,circle,draw,font=\sffamily\normalsize}]
            ...
            ;
        \end{tikzpicture}
        \caption{An example of a \tit{bipartite graph}. In particular, if we call the \curlyquotes{left} set of nodes $A$ and the \curlyquotes{right} one $B$, then $(A, B)$ is a bipartition of the graph.}
        % \label{}
    \end{figure}

    There are various types of graphs that can be bipartitioned. For example, every \tbf{tree} $T$ can be bipartitioned through a bipartition $(X, V(T) - X)$ by considering the following set of vertices $$X := \{v \in V(T) \mid \dist(r, v) \ \mathrm{even}\}$$ where $r$ is $T$'s root.
